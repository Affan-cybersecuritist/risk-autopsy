    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="glass-card w-[520px] max-w-[92vw] p-8 relative border-t-4 border-t-[#B23A48]"
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 bg-[#B23A48]/10 rounded-full">
              <ShieldAlert size={24} className="text-[#B23A48]" />
            </div>
          ...
          <div className="space-y-2 mb-6 max-h-[300px] overflow-y-auto pr-1">
            ...
            {gates.map(g => (
              <div
                key={g.name}
                className={`flex items-start gap-2 rounded-lg p-3 text-[13px] ${g.passed ? 'bg-neutral-50 border border-neutral-100' : 'bg-[#B23A48]/5 border border-[#B23A48]/20'}`}
              >
                {g.passed
                  ? <CheckCircle2 size={15} className="mt-0.5 shrink-0 text-emerald-600" />
                  : <span className="mt-0.5 shrink-0 font-bold text-[#B23A48]">✕</span>}
                <div>
                  <span className="font-bold text-neutral-800">{g.name}</span>
                  <span className="text-neutral-600"> — {g.detail}</span>
                  {!g.passed && <div className="text-[11px] text-[#B23A48] mt-0.5">needs {g.threshold}</div>}
                </div>
              </div>
            ))}
          </div>
          ...
          {failed.length > 0 && (
            <div className="bg-[#B23A48]/5 rounded-xl p-4 mb-6 border border-[#B23A48]/20">
              <p className="text-sm font-semibold text-[#B23A48] mb-1">
          ...
          <div className="flex gap-3 mt-2">
            ...
            <button
              onClick={onAccept}
              className="flex-1 py-3 px-4 rounded-xl text-white font-bold bg-[#B23A48] hover:bg-[#922A36] transition-colors shadow-lg shadow-[#B23A48]/20"
            >
